...
use crate::{
    ...
    CursorIcon, EventHandler, TouchPhase,
};
    ...
    F: 'static + FnOnce() -> Box<dyn EventHandler>,
{
    unsafe {
        ...
        crate::set_display(NativeDisplayData {
            ...
            sample_count: conf.sample_count.max(1) as u32,
            swap_interval: conf.platform.swap_interval,
            gfx_api: conf.platform.prefer_gfx_api,
            #[cfg(feature = "wgpu")]
            wgpu_backend: conf.platform.wgpu_backend,
        ...
        if conf.platform.prefer_gfx_api == crate::conf::GfxApi::OpenGl {
            let mut wgl = wgl::Wgl::new(&mut display);
            gl_context = Some(wgl.create_context(
                &mut display,
                conf.sample_count,
        ...
        if let Some(gl_context) = gl_context {
            (display.libopengl32.wglDeleteContext)(gl_context);
        }
        DestroyWindow(wnd);
    }
}
